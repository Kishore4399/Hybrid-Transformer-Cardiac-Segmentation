...
if __name__ == "__main__":
    ...
    if args.model == "HFTrans":
        train_dataset = CardiacDataset(args.dataset, patch_size = patch_size, subset='train', crossvalid=args.crossvalid, valid_fold=args.fold)
        valid_dataset = CardiacDataset(args.dataset, patch_size = patch_size, subset='valid', crossvalid=args.crossvalid, valid_fold=args.fold)
    elif args.model == "HFTrans2":
    ...
    while iteration <= total_iteration:
        ...
        if (iteration % train_interval == 0):
            ...
            if(iteration % (len(train_loader)*2000) == 0):
                torch.save({
                    'iteration': iteration+1,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
