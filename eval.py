...
def eval(model, device, path, name, patch_size, model_name, config, tta, resize, trained_size, output_dir):
    with torch.no_grad():
        ...
        img, label, header  = preprocessingCardiac2ch_eval([path,name])
        if "case19" in name:
            ref_path = path.replace("case19", "case1")
            ref_name = name.replace("case19", "case1")
            ref_img, ref_label, ref_header  = preprocessingCardiac2ch_eval([ref_path, ref_name])
            ref_img = ref_img[0, ...]
            ct_img = match_histograms(img[0, ...], ref_img)
            img[0, ...] = ct_img
        if resize:
...
if __name__ == "__main__":
    ...
    result_df.to_csv(f"cha2_case{args.fold}.csv")
