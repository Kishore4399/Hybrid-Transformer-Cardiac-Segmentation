...
def predict(model, device, path, name, patch_size, model_name, config, tta, resize, trained_size, output_dir):
    with torch.no_grad():
        model.eval()
        img, header  = preprocessingCardiac2ch_pred([path,name])
        if resize:
            img = zoom(img, (1, trained_size/img.shape[1], trained_size/img.shape[2], trained_size/img.shape[3]))
        input_img = makeTensor(img, device)
        ...
        pred[pred <= 0.5] = 0.0
        if resize:
            pred = zoom(pred.cpu().numpy(), (1,1,input_img.shape[2]/trained_size, input_img.shape[3]/trained_size, input_img.shape[4]/trained_size), order=0)
            pred = makeTensor(pred, device)
...
if __name__ == "__main__":
    ...
    output_dir = './predict/' + args.weight.split('/')[-1].split('.')[0]
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)

    test_imgs = [d for d in os.listdir(os.path.join(args.dataset,'images')) if d.startswith('case')]
    test_imgs.sort()
    print(test_imgs)
    test_path = [[args.dataset, name] for name in test_imgs]

    with tqdm(total=len(test_path), desc="predicting.....") as pbar:
        for path, name in test_path:
            predict(model, device, path, name, patch_size, args.model, config_vit, args.tta, args.resize, args.trained_size, output_dir)
            pbar.update(1)
