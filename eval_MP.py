...
def eval(model, device, path, name, patch_size, model_name, config, tta, resize, trained_size, output_dir):
    with torch.no_grad():
        ...
        # nrrd.write(os.path.join(output_dir,name.replace("ct.nrrd", "pred.nrrd")), pred, header=header)

        # img_ct, header  = nrrd.read(os.path.join(path, 'images', name))
        # two_channel = np.stack([img_ct, pred], axis=0)
        # one_channel = pred

        # ch2_save_dir = os.path.join(output_dir, "twoChannels", "images", )
        # MpPred_save_dir = os.path.join(output_dir, "MpPred", "images")
        # ct_save_dir = os.path.join(output_dir, "ct", "images")

        # os.makedirs(ch2_save_dir, exist_ok=True)
        # os.makedirs(MpPred_save_dir, exist_ok=True)
        # os.makedirs(ct_save_dir, exist_ok=True)

        # nrrd.write(os.path.join(ch2_save_dir, name), two_channel, header=header)
        # nrrd.write(os.path.join(MpPred_save_dir, name), one_channel, header=header)
        # nrrd.write(os.path.join(ct_save_dir, name), img_ct, header=header)
...
if __name__ == "__main__":
    ...
    for i, weight in enumerate(weights):
        ...
        test_imgs = [d for d in os.listdir(os.path.join(args.dataset,'images')) if f"case{fold}_" in d]
        test_imgs.sort()
